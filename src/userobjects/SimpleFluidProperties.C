...
SimpleFluidProperties::rho(Real pressure, Real temperature) const
{
 // return _density0 * std::exp(pressure / _bulk_modulus - _thermal_expansion * temperature);
  //* modify from Y. Zhao et al. http://dx.doi.org/10.1016/j.energy.2015.01.030
  Real one_over_density;
  one_over_density = 3.086 - std::pow(0.8990178 * (4014.15 - temperature), 0.147166)
                     - std::pow(0.39 * (658.15 - temperature ), -1.6) 
                     * (pressure - 225.5);
  return (1/one_over_density) * 1000;
}
